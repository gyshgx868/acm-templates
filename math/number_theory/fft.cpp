...
void transform(std::vector<std::complex<double> >& y, bool invert) {
  int len = y.size();
  rader(y);
...
std::vector<int> multiply(char a[], char b[]) {
  ...
  std::vector<std::complex<double> > x1(len, std::complex<double>(0, 0));
  std::vector<std::complex<double> > x2(len, std::complex<double>(0, 0));
