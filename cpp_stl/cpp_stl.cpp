...
// 可以插入value, 而不会破坏容器顺序的第一个位置 (插入后其和其后数后移),
// 指向键值>=key的第一个元素
lower_bound(v.begin(), v.end(), key);
// 可以插入value, 而不会破坏容器顺序的最后一个位置 (插入后其和其后数后移),
// 指向键值>key的第一个元素
...
// Example
char str[] = "now # is the $time for all # good men to co$$me to "
             "the # aid of the$ir country";
char delims[] = "#$";
char* result = NULL;
